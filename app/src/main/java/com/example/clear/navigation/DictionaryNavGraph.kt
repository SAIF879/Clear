...
fun NavGraphBuilder.dictionaryNavGraph(
    ...
    navigation(
        route = NavGraphs.Dictionary,
        startDestination = DictionaryScreens.SearchWordScreen.route
    ) {
        composable(route = DictionaryScreens.SearchWordScreen.route) {
            SearchWordScreen(navController = navController, viewModel = viewModel)
        }
    }
}
