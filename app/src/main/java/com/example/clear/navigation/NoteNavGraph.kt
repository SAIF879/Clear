...
fun NavGraphBuilder.noteNavGraph(navController: NavController , viewModel: NoteViewModel){
    navigation(route = NavGraphs.Note , startDestination = NoteScreens.CreateNotesScreen.route){
        composable(route = NoteScreens.CreateNotesScreen.route){ CreateNotesScreen( navController = navController , noteViewModel = viewModel)}
        composable(route = NoteScreens.EditNotesScreen.route){ EditNotesScreen(navController = navController , noteViewModel = viewModel)}
    }
}
